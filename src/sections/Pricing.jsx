...
const Pricing = () => {
    const [monthly, setMonnthly] = useState(false);
        ...
        <Element name='pricing'>
            <div className='container'>
                ...
                mx-auto border-l border-r border-s2 bg-s1/50 pb-40 pt-28 max-xl:max-w-4xl max-lg:border-none max-md:pb-32 max-md:pt-16'>
                    <h3 className='h3 max-lg:h4 max-md:h5 z-3 relative mx-auto mb-14 max-w-lg text-center text-4p max-md:mb-11 max-sm:max-w-sm'>Flexible pricing for teams of all sizes</h3>

                    <div className='relative z-4 flex mx-auto w-[375px] rounded-3xl border-[3px] border-s4/25 bg-s1/50 p-2 backdrop-blur-[6px]
                    max-md:w-[310px]'>
                        <button className={clsx('pricing-head_btn', monthly && 'text-p4')} onClick={() => setMonnthly(true)}>Monthly</button>
                        <button className={clsx('pricing-head_btn', !monthly && 'text-p4')} onClick={() => setMonnthly(false)}>Annual</button>
                        ...
                        relative border-2 p-7 max-xl:min-w-80 max-lg:rounded-3xl xl:w-[calc(33.33%+2px)]'>
                            ...
                            <div className={clsx('relative flex flex-col items-center', index === 1 ? 'pt-24' : 'pt-12')}>
                                ...
                                <div className='relative z-2 flex items-center justify-center'>
                                    <div className={clsx('h-num flex items-start', index === 1 ? "text-p3" : "text-p1")}>
                                        $ <CountUp start={plan.priceMonthly} end={monthly ? plan.priceMonthly : plan.priceYearly} duration={0.4} useEasing={false} preserveValue></CountUp>
                                    </div>
                            ...
                            {index === 1 && <p className="small-compact mt-9 text-center text-p3 before:content-['-'] before:mx-2.5 after:mx-2.5 after:content-['-']">Limeted time offer</p>}
                        </div>
                    ))}
                </div>
            </div>
